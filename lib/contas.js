...
/**
 * Soma `n` meses a uma data pura (`"YYYY-MM-DD"`), preservando o dia quando
 * cabe no mes destino e CLAMPANDO no ultimo dia quando nao cabe — "aluguel
 * todo dia 31" vencendo em fevereiro cai no dia 28/29, nunca rola pro dia 3
 * de marco. `Date` nativo do JS faz exatamente esse rolamento errado
 * (`new Date(2026, 1, 31)` vira 3 de marco), por isso a conta e feita nos
 * componentes inteiros, nunca com aritmetica de `Date` direta.
 *
 * Trabalha inteiramente em componentes de calendario (nunca em instante/fuso)
 * — mesma disciplina de `statusEfetivo()`, pela mesma razao: vencimento e
 * uma data, nao um ponto no tempo.
 *
 * @param {string} dataYMD `"YYYY-MM-DD"`
 * @param {number} n meses a somar (sempre positivo nos usos atuais)
 * @returns {string} `"YYYY-MM-DD"`
 */
export function adicionarMeses(dataYMD, n) {
  ...
  // Dia 0 do mes SEGUINTE = ultimo dia do mes destino (truque de calendario
  // classico, sem depender de tabela de dias-por-mes nem checar bissexto na
  // mao).
  const ultimoDiaDoMesDestino = new Date(Date.UTC(novoAno, novoMes, 0)).getUTCDate()
  const novoDia = Math.min(dia, ultimoDiaDoMesDestino)
