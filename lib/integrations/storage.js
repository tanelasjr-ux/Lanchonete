...
/**
 * Imagem do cardapio (poster/foto do cardapio impresso), servida no link/QR
 * publico que ja existe (aba "Cardapio Digital"). Mesmo padrao de uploadLogo,
 * bucket e limite proprios: e um poster de menu, normalmente mais denso e
 * maior que uma logo.
 */
export const CARDAPIO_IMAGEM_BUCKET = 'cardapios'
export const CARDAPIO_IMAGEM_MAX_BYTES = 5 * 1024 * 1024 // 5 MB
export const CARDAPIO_IMAGEM_MIME_PERMITIDOS = ['image/png', 'image/jpeg', 'image/webp']

const EXTENSAO_CARDAPIO_POR_MIME = {
  'image/png': 'png',
  'image/jpeg': 'jpg',
  'image/webp': 'webp',
}
...
/**
 * Valida e envia a imagem do cardapio de UMA empresa. Caminho sempre derivado
 * do `empresaId` vindo do token — nunca do corpo da requisicao.
 *
 * @param {string} empresaId
 * @param {Buffer} buffer
 * @param {string} mimeType
 * @returns {Promise<string>} URL publica da imagem
 */
export async function uploadCardapioImagem(empresaId, buffer, mimeType) {
  if (!isStorageConfigured()) {
    throw new Error('Storage nao configurado: defina SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY.')
  }
  if (!CARDAPIO_IMAGEM_MIME_PERMITIDOS.includes(mimeType)) {
    throw new Error('Formato nao suportado. Use PNG, JPG ou WEBP.')
  }
  if (!buffer?.length) throw new Error('Arquivo vazio.')
  if (buffer.length > CARDAPIO_IMAGEM_MAX_BYTES) {
    throw new Error(`Arquivo muito grande (${(buffer.length / 1024 / 1024).toFixed(1)} MB). Limite: 5 MB.`)
  }

  const supabase = await getSupabaseAdmin()
  const ext = EXTENSAO_CARDAPIO_POR_MIME[mimeType]
  // Nome fixo por empresa + upsert: trocar a imagem substitui a anterior em
  // vez de acumular arquivo orfao no bucket a cada troca.
  const caminho = `${empresaId}/cardapio.${ext}`

  const { error } = await supabase.storage
    .from(CARDAPIO_IMAGEM_BUCKET)
    .upload(caminho, buffer, { contentType: mimeType, upsert: true })
  if (error) throw new Error(`Falha ao enviar a imagem do cardapio: ${error.message}`)

  const { data } = supabase.storage.from(CARDAPIO_IMAGEM_BUCKET).getPublicUrl(caminho)
  // `?v=` derruba o cache do browser/CDN — sem isso, trocar a imagem mantendo
  // o mesmo caminho continuaria exibindo a versao antiga.
  return `${data.publicUrl}?v=${Date.now()}`
}

/** Remove a imagem do cardapio da empresa. Falha silenciosa: se o arquivo nao
 *  existir, nao ha o que apagar e isso nao e um erro. */
export async function removeCardapioImagem(empresaId) {
  if (!isStorageConfigured()) return
  const supabase = await getSupabaseAdmin()
  const caminhos = Object.values(EXTENSAO_CARDAPIO_POR_MIME).map((ext) => `${empresaId}/cardapio.${ext}`)
  await supabase.storage.from(CARDAPIO_IMAGEM_BUCKET).remove(caminhos)
}
