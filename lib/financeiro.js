...
/**
 * Compara um numero do periodo atual com o mesmo numero do periodo anterior.
 *
 * `delta_percent` e `null` quando o periodo anterior foi zero. Nao e zero e
 * nao e "infinito": crescer de R$ 0 para R$ 5.000 nao tem percentual com
 * significado, e mostrar "+100%" ou "+∞%" seria pior que mostrar so o valor
 * absoluto. Mesma regra de nao-inventar do CMV (`cmv_percent`) e do ponto de
 * equilibrio: indicador sem base real fica `null` e a tela mostra o estado
 * vazio certo.
 *
 * @param {number} atual
 * @param {number} anterior
 */
export function computeVariacao(atual, anterior) {
  ...
  return {
    ...
    anterior: round2(b),
    delta: round2(a - b),
    delta_percent: b !== 0 ? round2(((a - b) / Math.abs(b)) * 100) : null,
  }
}

/**
 * Janela imediatamente anterior, de mesma duracao, colada no inicio da atual.
 *
 * Regra unica para qualquer periodo (7 dias, 30 dias, mes, intervalo custom)
 * em vez de casos especiais por preset: "mes anterior" para um filtro de
 * 12/03 a 27/03 nao teria resposta obvia, e uma regra que so vale para alguns
 * presets vira pegadinha quando o dono usa o intervalo livre.
 *
 * @param {Date} inicio
 * @param {Date} fim
...
export function periodoAnterior(inicio, fim) {
  ...
  return {
    inicio: new Date(inicio.getTime() - duracao - 1),
