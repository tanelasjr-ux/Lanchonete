...
export const CANAIS = {
  ...
  delivery: 'Delivery',
  // `balcao` e `retirada` foram fundidos em `para_levar` na migration 0024.
  // Os rotulos continuam aqui porque o agrupamento e feito em memoria e pode
  // receber dado de um backup restaurado antes da migration — sem eles a
  // linha apareceria com a chave crua ("balcao") na tela do dono.
  balcao: 'Para levar (antigo balcão)',
  retirada: 'Para levar (antiga retirada)',
}
...
export function computeMargemPorCanal({ transacoes, pedidos }) {
  ...
  for (const p of pedidos || []) {
    // Mesmo default do POST /pedidos (normPedidoTipo em route.js): um pedido
    // sem `tipo` nao e "balcao legado", e simplesmente nasceu sem o campo.
    canalDoPedido.set(p.id, p.tipo || 'para_levar')
