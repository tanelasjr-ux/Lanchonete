...
import { createEntregadorRepository as mongoEntregador } from './mongo/entregadorRepository'
...
import { createContaRepository as mongoConta } from './mongo/contaRepository'
import { createAssinaturaRepository as mongoAssinatura } from './mongo/assinaturaRepository'
import { createAssinaturaPagamentoRepository as mongoAssinaturaPagamento } from './mongo/assinaturaPagamentoRepository'
import { createPlataformaAdminRepository as mongoPlataformaAdmin } from './mongo/plataformaAdminRepository'
...
import { createEntregadorRepository as sbEntregador } from './supabase/entregadorRepository'
...
import { createContaRepository as sbConta } from './supabase/contaRepository'
import { createAssinaturaRepository as sbAssinatura } from './supabase/assinaturaRepository'
import { createAssinaturaPagamentoRepository as sbAssinaturaPagamento } from './supabase/assinaturaPagamentoRepository'
import { createPlataformaAdminRepository as sbPlataformaAdmin } from './supabase/plataformaAdminRepository'
...
async function ensureMongoIndexes(database) {
  ...
  try {
    ...
    await database.collection('entregadores').createIndex({ empresa_id: 1, ativo: 1 })
    await database.collection('caixas').createIndex({ empresa_id: 1, status: 1 })
    ...
    await database.collection('caixas').createIndex(
      { empresa_id: 1 },
      { unique: true, partialFilterExpression: { status: 'aberto' } }
    )
    await database.collection('caixa_movimentos').createIndex({ empresa_id: 1, caixa_id: 1 })
    await database.collection('contas').createIndex({ empresa_id: 1, status: 1, vencimento: 1 })
    await database.collection('assinaturas').createIndex({ empresa_id: 1 }, { unique: true })
    await database.collection('assinatura_pagamentos').createIndex({ empresa_id: 1, pago_em: -1 })
    await database.collection('plataforma_admins').createIndex({ email: 1 }, { unique: true })
    // Index for estoque_habilitado queries (estoque feature)
    await database.collection('produtos').createIndex({ empresa_id: 1, estoque_habilitado: 1 }, { sparse: true })
  } catch (e) {
...
function buildMongoRepositories(database) {
  return {
    ...
    entregadorRepo: mongoEntregador(database),
    caixaRepo: mongoCaixa(database),
    caixaMovimentoRepo: mongoCaixaMovimento(database),
    contaRepo: mongoConta(database),
    assinaturaRepo: mongoAssinatura(database),
    assinaturaPagamentoRepo: mongoAssinaturaPagamento(database),
    plataformaAdminRepo: mongoPlataformaAdmin(database),
  }
}
...
function buildSupabaseRepositories(supabase) {
  return {
    ...
    entregadorRepo: sbEntregador(supabase),
    caixaRepo: sbCaixa(supabase),
    caixaMovimentoRepo: sbCaixaMovimento(supabase),
    contaRepo: sbConta(supabase),
    assinaturaRepo: sbAssinatura(supabase),
    assinaturaPagamentoRepo: sbAssinaturaPagamento(supabase),
    plataformaAdminRepo: sbPlataformaAdmin(supabase),
  }
}
